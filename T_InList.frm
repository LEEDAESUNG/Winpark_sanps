...
Begin VB.Form T_InList 
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "T_InList.frx":0014
      Height          =   9000
      Left            =   135
      OleObjectBlob   =   "T_InList.frx":0028
      TabIndex        =   10
      Top             =   540
      Width           =   14925
   End
End
Attribute VB_Name = "T_InList"
...
Data1.RecordSource = "SELECT * FROM t-in WHERE (입차일자>='" & Format(DTPicker1.Value, "yyyy-mm-dd") & "') AND (입차일자<='" & Format(DTPicker2.Value, "yyyy-mm-dd") & "') ORDER BY 포스번호, 입차일자, 입차시간, 카드번호"
Data1.Refresh
...
Record_Source = "SELECT * FROM t-in WHERE 입차일자 = '" & Format(Now, "yyyy-mm-dd") & "' ORDER BY 포스번호, 입차일자, 입차시간, 카드번호"
